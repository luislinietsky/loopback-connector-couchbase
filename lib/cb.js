...
exports.initialize = function initializeDataSource(dataSource, callback) {
	...
	var options = {
		...
		openConnectionDuration : s.openConnectionDuration || 20000,
		idleConnectionDuration : s.dileConnectionDuration || 180000, 
	};
	options.connectUrl = options.host;//"couchbase://" + options.host;  //do not attach any port number such as 8901, will cause error code 24 which means "Error while establishing TCP connection"
...
CouchbaseDB.prototype.connect = function (callback) {
  ...
  if (self.db) {
	...
	var clusterNodes = [];
	if(Array.isArray(self.settings.connectUrl)){
		clusterNodes = self.settings.connectUrl;
	}else{
		var urls = self.settings.connectUrl.split(",");
		for(var i = 0 ; i < urls.length ; i++){
			var parts = urls[i].split(":");
			var port = parts.length >= 2 ? parts[1] : self.settings.port;
			clusterNodes.push([parts[0], port]);
		}
	}
	self.myCluster = [];
	for(var i = 0 ; i < clusterNodes.length; i++){
		var clusterNode = new couchbase.Cluster("couchbase://" + clusterNodes[i]);
		clusterNode.n1qlurl = clusterNodes[i][0] + ":" + self.settings.n1qlport;
		self.myCluster.push(clusterNode);
	}		
	callback && callback(null, self.db);
  }
};

CouchbaseDB.prototype.getCluster = function(){
	if(Array.isArray(this.myCluster)){
		return this.myCluster[parseInt(this.myCluster.length * Math.random())];
	}else{
		return this.myCluster;
	}
}

CouchbaseDB.prototype.openBucket = function(bucketName, password, callback){
	var self = this;
	if(!self.openBuckets){
		self.openBuckets = {};
	}
	if(self.openBuckets[bucketName] && ! self.openBuckets[bucketName].isExpired() && self.openBuckets[bucketName].bucket.connected){
		process.nextTick(callback);
		self.openBuckets[bucketName].resetTtl();
		return self.openBuckets[bucketName].bucket;
	}else{
		if(self.openBuckets[bucketName] && self.openBuckets[bucketName].isExpired() && self.openBuckets[bucketName].bucket.connected){
			// self.openBuckets[bucketName].bucket.disconnect();
		}
		...
		var bucket = cluster.openBucket(bucketName, password, function(err){
			if(err){
				...
				self.openBuckets[bucketName] = new function(){
					_self = this;
					this.resetTtl = function(){
						_self.expiresAt = Date.now() + self.settings.openConnectionDuration;
						_self.disconnectAt = Date.now() + self.settings.idleConnectionDuration;
						setTimeout(function(){
							if(_self.disconnectAt < Date.now() && _self.bucket.connected){
								_self.bucket.disconnect();
							}else{
							}
						}, self.settings.idleConnectionDuration + 1000);
					}
					this.isExpired = function(){
						return Date.now() > _self.expiresAt;
					}
					this.resetTtl();
					this.bucket = bucket;
				}
		...
		bucket.operationTimeout = self.settings.operationTimeout;
		bucket.connectionTimeout = self.settings.connectionTimeout;
		bucket.n1qlurl = cluster.n1qlurl;
		return bucket;
	}
}
...
var cbquery = function (sql,selfThis, callback) {
	var self = selfThis;
	var myBucket = self.openBucket(self.settings.bucket, self.settings.password, function(err){
		if(err){
			callback(err,null);
			return;
		}
		...
		myBucket.enableN1ql([myBucket.n1qlurl]);
			...
			if(res != null){
				if( Object.prototype.toString.call( res ) === '[object Array]' ) {
					...
					for(var i = 0 ; i < res.length ; i++){
						var row = res[i];
						if(row[self.settings.bucket] && Object.keys(row).length == 1){
							res2.push(row[self.settings.bucket]);
						}else{
							res2.push(row);							
						}
					}
	...
	var myBucket = self.openBucket(self.settings.bucket, self.settings.password, function(err){
		...
		myBucket.get(id,function(err, result) {
			...
			for(var i = 0 ; i < result.length ; i++){
				modelsFixed.push(self.fromDatabase(model, result[i]));
			}
			// debug("CouchbaseDB.prototype.find()  : find result is:  "  + JSON.stringify([err, result]) );
			debug("CouchbaseDB.prototype.find()  : find result is:  "  + JSON.stringify([err, modelsFixed]) );
			// callback(err,result);
			callback(err,modelsFixed);
			return;
		});
	});
};
...
CouchbaseDB.prototype.updateOrCreate = function updateOrCreate(model, data, callback) {
	...
	var myBucket = self.openBucket(self.settings.bucket, self.settings.password, function(err){
		...
		updateOneDoc(self,myBucket,createDataBuilderObj.getIdKey(),model,createDataBuilderObj.getPreparedData(),function(err, result){
			callback(err,result);
		});
	});
};
...
CouchbaseDB.prototype.updateAttributes = function updateAttrs(model, id, data, callback) {
	...
	var myBucket = self.openBucket(self.settings.bucket, self.settings.password, function(err){
		...
		updateOneDoc(self,myBucket,idkey,model,data,function(err, result){
			callback(err,result);
		});
	});
};
